...
export const metadata:Metadata={
  ...
  title:'Priyansh Dobariya — Applied AI Engineer',description:profile.description,
  applicationName:'Priyansh Dobariya Portfolio',authors:[{name:profile.name,...(siteUrl?{url:siteUrl}:{})}],creator:profile.name,
  keywords:['Priyansh Dobariya','Applied AI Engineer','LLM systems','RAG engineer','AI agents','AI automation','LangGraph','Python','FastAPI'],
  ...
  openGraph:{type:'website',locale:'en_IN',siteName:'Priyansh Dobariya',title:'Priyansh Dobariya — Applied AI Engineer',description:profile.description,...(siteUrl?{url:siteUrl,images:[{url:`${siteUrl}/og.png`,width:1200,height:630,alt:'Priyansh Dobariya — Applied AI Engineer'}]}:{})},
  twitter:{card:'summary_large_image',title:'Priyansh Dobariya — Applied AI Engineer',description:profile.description,...(siteUrl?{images:[`${siteUrl}/og.png`]}:{})},
...
export default function RootLayout({children}:Readonly<{children:React.ReactNode}>){return <html lang="en" suppressHydrationWarning className={`${sans.variable} ${heading.variable} ${serif.variable}`}><head><script dangerouslySetInnerHTML={{__html:themeScript}}/><link rel="preload" as="image" href="/developer-sprites.webp"/></head><body>{children}</body></html>;}
